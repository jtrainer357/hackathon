        ...
        <WidgetContainer
            ...
            headerAction={
                <div className="flex items-center gap-2">
                    ...
                    <Badge variant="secondary" className="bg-muted text-muted-foreground hover:bg-muted">14 Appointments</Badge>
                </div>
            }
            contentClassName="flex flex-col gap-4"
        >
            {/* Priority Action Card */}
            <div className="bg-vitality-5 rounded-xl px-6 py-4 flex items-center justify-between border border-border/50 shrink-0">
                ...
                <Button variant="default" className="rounded-full px-6 font-bold bg-vitality-1 hover:bg-vitality-2 text-white border-0 shadow-lg shadow-vitality-1/20">Begin Check-in</Button>
            </div>
                    ...
                    {appointments.map((apt) => (
                        <div
                            ...
                            className={cn(
                                ...
                                apt.status === 'ENDED' ? 'bg-core-4/30 hover:bg-core-4/50' :
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3 w-64 shrink-0 mr-8">
                                ...
                                <div>
                                    <div className="font-bold text-sm leading-none mb-0.5 text-foreground">{apt.patient}</div>
                                    {apt.type && <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider opacity-80">{apt.type}</div>}
                                </div>
                            </div>

                            <div className="w-48 font-medium text-sm px-4 shrink-0 text-muted-foreground">
                                {apt.doctor}
                            </div>
                            ...
                            <div className="flex-1 flex justify-center">
                                <Badge
                                    variant={
                                        apt.status === 'ENDED' ? 'secondary' : 'default'
                                    }
                                    className={cn(
                                        "rounded-md px-2 py-0.5 text-[10px] uppercase tracking-wider font-bold border-0",
                                        apt.status === 'ENDED' && "bg-core-3 text-core-1",
                                        (apt.status === 'IN PROGRESS' || apt.status === 'CHECKED IN') && "bg-vigor/10 text-vigor",
