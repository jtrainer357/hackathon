...
export function DashboardSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
    ...
    return (
        <Sidebar collapsible="icon" className="relative !border-r-0 [&_[data-sidebar=sidebar]]:bg-transparent bg-transparent" {...props}>
            <SidebarHeader className="pt-widget-padding-y">
                <SidebarMenu>
                    <SidebarMenuItem>
                        <SidebarMenuButton size="lg" className="justify-center hover:bg-transparent cursor-default">
                            <div className="flex aspect-square size-10 items-center justify-center rounded-xl bg-card text-growth-1 shadow-sm transition-shadow">
                        ...
                        return (
                            <SidebarMenuItem key={item.title}>
                                <SidebarMenuButton
                                    ...
                                    isActive={isActive}
                                    tooltip={item.title}
                                    className="justify-center h-10 w-10 mx-auto rounded-xl border border-sidebar-border/80 data-[active=true]:border-transparent data-[active=true]:bg-growth-1 data-[active=true]:text-white hover:bg-muted hover:text-growth-1 data-[active=true]:hover:text-white transition-all duration-300 ease-in-out"
                                >
                                    <Link href={item.url}>
            ...
            <div className="fixed bottom-8 left-0 w-[var(--sidebar-width-icon)] hidden md:flex flex-col items-center z-20 pointer-events-none pb-0">
                <SidebarMenu className="pointer-events-auto flex flex-col items-center gap-4 w-full">
                    <SidebarMenuItem className="w-full flex justify-center">
                        <SidebarMenuButton size="lg" className="justify-center w-10 h-10 mx-auto p-0 rounded-xl border border-sidebar-border/80 hover:bg-muted hover:text-growth-1">
                            <Bell className="size-5 transition-all" />
                        </SidebarMenuButton>
                    </SidebarMenuItem>
                    <SidebarMenuItem className="w-full flex justify-center">
                        <div className="flex justify-center">
                            <Avatar className="h-8 w-8 cursor-pointer hover:ring-2 hover:ring-growth-4 transition-all">
